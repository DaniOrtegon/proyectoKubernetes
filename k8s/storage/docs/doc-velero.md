# Backup completo del clúster con Velero (`velero.yaml`)
...
**NetworkPolicy de egress** — en este proyecto, `network-policy.yaml` define políticas restrictivas para todos los namespaces. Velero necesita reglas explícitas porque: (1) accede a un namespace externo (`storage`/MinIO), (2) accede a la API server de Kubernetes desde dentro del clúster, y (3) necesita resolución DNS para las URLs internas.
...
| Archivo | Relación |
|---|---|
| `minio.yaml` | MinIO debe estar operativo antes de ejecutar `velero-bucket-setup`; proporciona la instancia S3 |
